...
def get_metrics(tool, df, query2args_dict):
    ...
    df['exact_match_wiki'] = df.apply(exact_matching, 
                                       args=("preds_wiki",), axis=1)
    
    df0 = df.drop_duplicates(subset =["base_question", "query", "preds_wiki"],
    ...
    df_final = pd.merge(left=df, right=df1, how='inner', left_on=common, right_on=common)
